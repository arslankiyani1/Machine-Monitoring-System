...
public class CustomerDtoValidator : AbstractValidator<AddCustomerDto>
{
    ...
    .When(x => !string.IsNullOrWhiteSpace(x.State));
        ...
        RuleForEach(x => x.Shifts)
            .ChildRules(sh =>
            {
                sh.RuleFor(s => s.ShiftName)
                    .NotEmpty().WithMessage("ShiftName is required.")
                    .MaximumLength(50).WithMessage("ShiftName must not exceed 50 characters.");
            })
    ...
    private const string TimeFormatRegex = @"^([01]\d|2[0-3]):([0-5]\d)$";
